...
export class Operacion extends RestController implements OnInit{
    ...
    ngOnInit(){
        if (this.myglobal.existsPermission('93')) {
            ...
            if(localStorage.getItem('view2'))
                this.view = JSON.parse(localStorage.getItem('view2'));
    ...
    public view = [
        ...
        {'visible': true, 'position': 7, 'title': 'Peso descargado', 'key': 'neto'},
    ...
    ordenView(){
        ...
        localStorage.setItem('view2',JSON.stringify(this.view))
    }
    setVisibleView(data){
        this.view.forEach(key=>{
            if(key.key == data.key)
